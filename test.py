...
dataf = pd.read_csv("Datasets/OECD_Original/ICIO2023_2019.csv", index_col=0)
...
edge_to_rem = [(a, b) for a, b, attrs in G.edges(data=True) if attrs["weight"] <= 2500 or a[:3] == b[:3]]
...
def display_using_plotly(graph):
    pos = nx.spring_layout(graph)
    node_x = [x for x, y in pos.values()]
    node_y = [-y for x, y in pos.values()]
    edge_x = [x for n0, n1 in graph.edges for x in (pos[n0][0], pos[n1][0], None)]
    edge_y = [y for n0, n1 in graph.edges for y in (-pos[n0][1], -pos[n1][1], None)]
